...
from src.agents import image_desc_generator, image_generator, image_query_creator, image_search_agent

logger = logging.getLogger(__name__)

async def generate_single_image( headline: str, session_id: str,model:str) -> Dict[str, str]:
...
async def fetch_multiple_images(headline: str, session_id: str, reference_image:bytes = None) -> List[Dict[str, str]]:
    """Process a single image: generate, save, create HTML, and capture screenshot
    ...
    try:
        ...
        # Generate image from description
        image_list = await image_search_agent(query=image_query["queries"][0], reference_image=reference_image)
...
if __name__ == "__main__":
    ...
    result = asyncio.run(fetch_multiple_images(headline="Rahul Gandhi Alleges Voter Fraud in Bengaluru", reference_image=None, session_id=session_id))
